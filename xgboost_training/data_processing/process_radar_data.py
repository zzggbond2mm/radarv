...
def process_and_extract_points(matlab_path, track_path, folder_name):
    ...
    if found_mat_indices:
        found_array = mat_da[list(found_mat_indices)]
        found_df = pd.DataFrame(found_array, columns=COLUMN_NAMES)
        output_path = os.path.join(output_dir, 'found_points.csv')
        found_df.to_csv(output_path, index=False)
        print(f"  - [{folder_name}] 已保存 {len(found_df)} 个匹配点到 found_points.csv")
    else:
    ...
    if unfound_mat_indices:
        ...
        output_path = os.path.join(output_dir, 'unfound_points.csv')
        unfound_df.to_csv(output_path, index=False)
        print(f"  - [{folder_name}] 已保存 {len(unfound_df)} 个未匹配点到 unfound_points.csv")
    else:
...
def run_tasks_in_parallel(tasks):
    ...
    使用线程池并行执行所有数据处理任务。
    """
    print(f"--- 第5步: 开始多线程处理 {len(tasks)} 个任务 ---")
...
def main():
    ...
    if not os.path.isdir(SOURCE_DATA_DIR):
        print(f"错误: 源数据目录 '{SOURCE_DATA_DIR}' 不存在。")
        print("请在脚本中将 'SOURCE_DATA_DIR' 变量修改为正确的路径。")
        return

    # 第1步: 扫描源目录，找到所有有效的文件对
    all_pairs = scan_for_file_pairs(SOURCE_DATA_DIR)
    if not all_pairs:
        print("在源目录中没有找到任何文件对，程序退出。")
        return
    ...
    print(f"--- 数据集划分完成: {len(train_pairs)} 对用于训练, {len(test_pairs)} 对用于测试。 ---\n")

    # 第3步: 在当前目录下创建 train/test 结构并复制文件
    create_dataset_structure(train_pairs, "train", OUTPUT_DIR)
    create_dataset_structure(test_pairs, "test", OUTPUT_DIR)
